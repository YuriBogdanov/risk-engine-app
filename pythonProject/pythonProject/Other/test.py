    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for owner in owners_list:
                ...
                # Огромный плюс Moralis: они уже посчитали нормальный баланс за нас!
                balance = owner.get("balance_formatted")

                # Достаем метку кошелька, если она есть (например, биржа или смарт-контракт)
                label = owner.get("owner_address_label", "Обычный кошелек")

                # Процент от общей эмиссии токена
                percent = owner.get("percentage_relative_to_total_supply", 0)

                print(f"Кошелек: {address} [{label}]")
                print(f"Баланс:  {balance} ({percent}% от всех токенов)")
                print()
